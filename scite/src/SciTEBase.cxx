...
bool SciTEBase::ProcessCommandLine(const GUI::gui_string &args, int phase) {
	...
	for (size_t i = 0; i < wlArgs.size(); i++) {
		...
		if (IsSwitchCharacter(arg[0])) {
			...
			if (!PreOpenCheck(arg)) {
				...
				toBeOpenFiles.push_back(GUI::gui_string(arg));
			}
	...
	if (phase == 1) {
		...
		for(int i =0; i< toBeOpenFiles.size(); i++){
			Open(toBeOpenFiles[i].c_str(), static_cast<OpenFlags>(ofQuiet | ofSynchronous));
		}
		toBeOpenFiles.clear();
